...
created-date range), and a real database search, not client-side
...
these three buttons at all (nor can it call the endpoint directly; the
API itself enforces this with a 403).
...
Open `http://localhost:3000` and sign in with the demo credentials from
